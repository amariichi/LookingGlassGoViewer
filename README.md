...
**A:** 非圧縮で出力しているため、サイズが大きいです。試しに Gimp で "Save color values from transparent pixels" にチェックを入れ、圧縮レベルを 9 にして右半分にデプス推定結果が付いた画像をPNG形式で圧縮して保存し、このアプリで読み込んでみました。どのようなデプス情報が保持されているかは未確認ですが、デプスの相互関係が大きく破綻する様子は観られませんでした。圧縮した画像を使用するとアプリの動作が軽くなりますが、画質は低下します。
